"""
Phase 1 Test Script - Fixed Version
...
def test_dashboard_queries():
    ...
    if user:
        ...
        # Test category performance (FIXED: use tuple indices)
        ...
        if categories:
            print(f"Strongest category: {categories[0][0]} ({categories[0][1]}%)")
            if len(categories) > 1:
                print(f"Weakest category: {categories[-1][0]} ({categories[-1][1]}%)")
            else:
                print(f"Weakest category: {categories[0][0]} ({categories[0][1]}%)")
        else:
        ...
        print("✓ Dashboard queries work correctly")
