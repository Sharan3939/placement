...
With XP, Badges, Dashboard, Adaptive Difficulty, and Company Mocks
"""
...
def get_company_mock_questions(company_quiz_id):
    """
    Fetch company mock questions with proper distribution:
    ...
    Returns dict with sections and coding questions.
    """
    import random
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    for section in sections:
        ...
        easy_count = section['difficulty_easy']
        medium_count = section['difficulty_medium']
        hard_count = section['difficulty_hard']
        ...
        # If not enough questions, fetch more regardless of difficulty
...
def company_mock_test(company_name):
    """Company mock test with section-wise questions"""
    conn = get_db_connection()
    cursor = conn.cursor()
    
    # Map URL-friendly name to quiz name
    company_map = {
        'tcs': 'TCS Pattern',
    ...
    # Get questions with distribution
    questions_data = get_company_mock_questions(quiz['id'])
    
    conn.close()
    
    # Flatten all MCQ questions for the template
    all_mcqs = []
...
@login_required
def company_mocks():
    """Company-wise mock tests page - TCS, Infosys, Wipro, Amazon patterns"""
    conn = get_db_connection()
    cursor = conn.cursor()
    
    # Fetch all company quizzes (name ends with 'Pattern')
    cursor.execute("SELECT id, name FROM quizzes WHERE name LIKE '%Pattern' ORDER BY id")
    quizzes = cursor.fetchall()
    
    companies = []
    for q in quizzes:
        # Map quiz name to URL key
        name = q['name'].replace(' Pattern', '')
        key = name.lower()
        companies.append({
            'id': q['id'],
            'name': name,
    ...
    return render_template("company_mocks.html", companies=companies)
...
def evaluate_code():
    ...
    # Security check - reject dangerous imports/functions
